...
TAD-LHC is configured via CLI flags. The most important parameters are exposed under the
`benchmark` subcommand (examples below).

## Usage Examples

### Example 1: Basic Benchmark (Quick)

```bash
# Quick benchmark (recommended)
python3 TAD-LHC.py benchmark --quick
```

**Sample Output:**
```
2026-02-04 00:58:47 - TAD-LHC.Benchmark - INFO - BENCHMARK RESULTS
2026-02-04 00:58:47 - TAD-LHC.Benchmark - INFO - Total events processed: 2000
2026-02-04 00:58:47 - TAD-LHC.Benchmark - INFO - Test F1: 0.80
2026-02-04 00:58:47 - TAD-LHC.Benchmark - INFO - Test PR-AUC: 1.00
2026-02-04 00:58:47 - TAD-LHC.AnomalyDetector - INFO - Topological evolution visualization saved to topological_evolution_*.png
...
### Validation Report (KS/χ² vs ATLAS Z→μμ)

Use `--validation-report` to generate a JSON file with KS and χ² statistics comparing
the simulated Z→μμ mass distribution against ATLAS public plots in the 80–100 GeV range.

```bash
python3 TAD-LHC.py benchmark \
  --validation-report validation_report.json
```

The output file contains:
- `ks`: Kolmogorov–Smirnov statistic (shape difference)
- `chi2`: χ² statistic (binned deviation)
- `mass_range`: [80.0, 100.0]

### Example 2: Alpha-complex + Validation Report

```bash
# Alpha-complex (physically scaled) + validation report (KS/χ² vs ATLAS Z→μμ)
python3 TAD-LHC.py benchmark \
  --tda-complex alpha \
  --alpha-max 0 \
  --alpha-max-points 50 \
...
**Sample Output:**
```
2026-02-04 01:02:44 - TAD-LHC.Benchmark - INFO - Validation report saved to validation_report.json
...
### Example 3: Collision Plots

Use `--plots` to save a PNG with collision visualizations (mass, pT, mass vs pT, η).

```bash
python3 TAD-LHC.py benchmark \
  --plots collision_plots.png
```

The PNG will be saved at the path you provide. Open it with any image viewer.
