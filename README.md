...
# Compute Betti numbers (requires persistence first)
_ = hypercube_builder.compute_persistence_diagram()
...
Run the benchmark via the `benchmark` subcommand. Quick mode is the default preset.

```bash
# Quick benchmark (recommended)
python3 TAD-LHC.py benchmark --quick

# Full benchmark (heavy)
python3 TAD-LHC.py benchmark --full-benchmark

# Alpha-complex (physically scaled) + validation report
...
python3 TAD-LHC.py benchmark \
  ...
  --tau 0.1 \
  --grid 0.5,1.0,2.0 \
  --rips-max-edge-length 0.5 \
  --rips-sparse 0.1 \
  --rips-max-points 2000 \
  --seed 42 \
  --cache-precision 3 \
  --report benchmark_report.json

# Collision plots + benchmark report
python3 TAD-LHC.py benchmark \
...
Notes:
- `bins` grows memory as `bins^5`; values above 30 can exhaust RAM.
- `cache-precision` controls window-hash rounding (higher = fewer cache hits, more compute).
- `--validation-report` writes KS/χ² validation vs ATLAS Z→μμ in 80–100 GeV.
